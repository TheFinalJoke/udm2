...
impl server::udm_service_server::UdmService for UdmService {
    async fn add_fluid_regulator(&self, _request: Request<service_types::AddFluidRegulatorRequest>) -> Result<Response<service_types::AddFluidRegulatorResponse>, Status> {
        let reply = service_types::AddFluidRegulatorResponse{
            fu_id: 132
        };
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    Server::builder()
        .add_service(server::udm_service_server::UdmServiceServer::new(udm_service))
        .serve(addr)
        .await?;
    Ok(())
}
